...
from comments.models import Comment, MockPost
from comments.tools import get_captcha_key, split_on_pages
...
def block_comments(request, sorter):
    ...
    zero_comments = all_comments.filter(level=0).order_by(sorter, "-created_on")

    items_on_page = 10
    page = split_on_pages(
        request=request, queryset=zero_comments, items_on_page=items_on_page
    )

    # add subcomments to zero comments on page
    page_tree_comments = [
        all_comments.filter(tree_id=i.tree_id) for i in page.object_list
    ]
...
def reply_form_comment(request, comment_id=None):
    ...
    return render(
        request, "comments/nodes/form_new_comment.html", {"comment_form": form}
    )
